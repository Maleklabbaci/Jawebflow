(function () {
  ...
  function initWidget() {
    ...
    // --- FIX CRITIQUE: Origine reelle de ton backend Cloudflare ---
    var apiOrigin = '';
    try {
      ...
      console.warn('[Jawebflow] Impossible de detecter l origin, fallback.');
    }
    ...
    function askAssistant(userMessage, onDone) {
      fetch((apiOrigin || '') + '/api/chat', {
        ...
        .then(function (data) {
          var reply = (data && (data.reply || data.text || data.message || data.response)) ||
            "Merci pour votre message ! Notre équipe vous recontactera rapidement.";
          onDone(reply);
        })
        .catch(function (err) {
          console.error('[Jawebflow] Error chat API:', err);
          onDone("Notre assistant est momentanément indisponible. Merci de réessayer dans un instant.");
        });
    }
    ...
    function render() {
      ...
      if (isOpen) {
        ...
        subtitle.textContent = 'En ligne \u00b7 Réponse IA immédiate';
        ...
        if (brandingEnabled) {
          ...
          branding.textContent = 'Développé par JawebFlow';
  ...
  // Lancement garanti sans crash DOM
